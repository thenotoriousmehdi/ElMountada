...
class UsersView
{
    ...
    echo '<form action="/ElMountada/users/ShowUsers" method="POST" class="flex gap-4 items-end">'; 
     ...
     echo '</div>';
        ...
        if (empty($users)) {
            ...
            foreach ($users as $user) {
                echo "<tr class='border-t border-primary/5 hover:bg-primary/10' onclick=\"window.location='/ElMountada/users/showProfilePage?id=" . htmlspecialchars($user->id) . "'\">";
                ...
                echo "<form action='/ElMountada/users/deleteUser' method='POST'onsubmit='return confirm(\"Etes vous sure de vouloir supprimer cet utilisateur ?\")' >";
                ...
                echo "<form action='/ElMountada/users/blockUser' method='POST'onsubmit='return confirm(\"Etes vous sure de vouloir bloquer cet utilisateur ?\")' >";
                echo "<input type='hidden' name='user_id' value='" . htmlspecialchars($user->id) . "'>";
                echo "<button type='submit' class='bg-black text-white px-4 py-2 rounded-lg'>Bloquer</button>";
                echo "</form>";

                echo "<form action='/ElMountada/users/makeMember' method='POST'onsubmit='return confirm(\"Etes vous sure de vouloir rendre cet utilisateur membre ?\")' >";
                echo "<input type='hidden' name='user_id' value='" . htmlspecialchars($user->id) . "'>";
